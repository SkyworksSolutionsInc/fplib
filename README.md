...
fplib is a synthesizable SystemVerilog library for working with fixed point (FP) numbers. This library abstracts out the error-prone task of keeping track of the integer and fractional bits (the binary point) when performing math and converting FP numbers, namely:

- add, subtract, and multiply operations
- changing the number of int/frac bits ('resizing') to increase or decrease the range / resolution
- converting float constants (e.g. FIR/IIR filter coefficients) to FP
- converting FP to float (for debugging in the waveform viewer)

It does so by (ab)using SystemVerilog interfaces  to encapsulate a normal `logic` vector as well as the parameters for the number of int/frac bits into [a single object](./rtl/fp_if.sv) (in the absence of proper language support like [in VHDL](https://github.com/FPHDL/fphdl)). An effort is made to minimize the usage of macros while still keeping the syntax as clean as possible within the (many) limitations of the SV language.

In RTL simulations, the interface also holds the float (`real`) representation of the number, which is a significant time-saver when using the waveform viewer during debug / DV. In the screenshot below from Cadence SimVision showing a signed fixed point interface ("sfp signal"), `fval = 0.8741` is the float representation of the 24-bit logic vector `val = 7,332,698` in signed 1.23 (`iw = 1`, `qw = 23`) format, as commonly used in audio DSP:
...
Suppose we needed to design the data-path for a generic linear interpolation module, i.e. y = m*x + b, where y, m, x, and b are fixed point numbers with arbitrary int/frac widths. This is also known as a FMAC (fused MAC) operation.

A standard implementation of such a module and the instantiation of it would look like [interp_no_fplib.sv](./examples/interp_no_fplib.sv). The module is over 20 lines of error-prone code, where all we wanted to was a multiply and an add. if we did not parametrize all the widths we would not need as many lines of code, but that would come at the expense of losing reusability and even less code readability.
...
For a full list of the modules contained in this library see [the HTML documentation](./docs/index.html).
For a quick start guide that touches on most of what you need to know, see below.

## Tests

A basic [SV test bench](./tests/test_fplib.sv) has been written to verify the correctness of the math against a reference SV model. The script [gen_pkg.m](./tests/gen_pkg.m) generates a SV package which contains the randomized word lengths and real numbers used inside the TB. The test output is checked in as [xrun.log](./tests/xrun.log).
...
Define a `ufp` (unsigned FP) and a `sfp` (signed FP) signal, which are really SV interfaces parameterized with the desired `iw` and `qw`:
...
Assign an integer to `val` inside `y`, which actually holds the data:
...
The float representation of the fixed point number is held in `fval` (a `real`) in RTL simulations for easier debug and DV. So in this case, `x.val = 7066060` and `x.fval = 0.8423399925`.
...
```
The `clipping` indicator goes high when `x` is being clipped to max/min value that can fit in a s0.23.
...
Convert from unsigned to signed and resize to the desired format with `ufp_to_sfp` and `sfp_to_ufp`. For example, make a unsigned number signed by adding a zero sign bit and drop an LSB to reduce resolution in one go with:
...
- There is a `ufp_*` and a `sfp_*` variant of the add/sub/mult/resize modules. The `ufp_*` variants have all unsigned inputs/output and `sfp_*` variants have all signed inputs/output, with the exception of `ufp_sub` which has unsigned inputs but a signed output.
...
- The top level module of the simulation cannot have `ufp` / `sfp` interfaces in the port list. This is a SV limitation. You need to write a wrapper to convert the interfaces to normal vectors. For example, for a module with a `ufp` input x and `ufp` output y:
...
- If there are any X or Z bits in `val`, the `fval` real is set to float NaN.
...
2. Alternatively, you can define a `ufp_arr` / `sfp_arr`, which encapsulate a 1D unpacked array:
